...
public:
    ...
    {
        if(!this->root)//check if there is no root
        {
        ...
        while (!my_queue.isEmpty())
        {
            TreeNode<T> *node=my_queue.dequeue();//dequeue node and enqueue its childern
        ...
        return false;// Gets the catch of when there is a root of null but there are children to be added
    }
    void deSerialize(string tree)//deSearialize takes in a string and inserts it into a tree
    {
        string tmp;
    ...
    string serialize() {}//given a string, check if it is serialized
    bool isValidSerialization(string str)
    {
        string tmp;
        for(int i=0; i<str.length(); i++)//loop through the string 
        {
            if(str[i]==' ') return false;//if there is space(not valid)
            if(str[i]!=',')//if it is number or character add that tmp str
            {
                tmp+=str[i];
            }
            else
