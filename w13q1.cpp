...
public:
    ...
    void adjMatrix()//2d array checking links between vertices
    {
        ListNode<GraphNode<T>*>* cur=vertex->gethead();//pointer set to the first node
        for(int i=0; i<count; i++)//count keeps track of the number of verteces
        {
            ListNode<GraphNode<T>*>* node=vertex->gethead();
            for(int j=0; j<count; j++)//loop through the vertices
            {

                if(isLinked(cur->getData(),node->getData())) cout<<1;//check if cur is linked to node
                else cout<<0;
                node=node->getNext();
            }
            cur=cur->getNext();
    ...
    void adjList()//array consisting of addresses of all the linked lists
    {
        ListNode<GraphNode<T>*>* cur=vertex->gethead();
        for(int i=0; i<count; i++)
        {
            ...
            for(int j=0; j<count; j++)
            {
                ...
                if(isLinked(cur->getData(),node->getData())) cout<<j<<" ";
                node=node->getNext();
            }
            cur=cur->getNext();
